...
void LogAnalysis::getin_ip() {
    ...
    //sort by value
    for(cnt_cstidx = cnt.begin();
        cnt_cstidx != cnt.end();
        ++cnt_cstidx) {
        cnt_reverse.insert( make_pair(cnt_cstidx->second,
                                      cnt_cstidx->first) );
    }
}
...
void LogAnalysis::getout_rst() {
    for (cnt_reverseidx = cnt_reverse.rbegin();
         cnt_reverseidx != cnt_reverse.rend();
         ++cnt_reverseidx) {
        fip->GetAddressByIp(cnt_reverseidx->second.c_str(),
                            addr.country, addr.location);
        outfile << left << setw(15)
                << cnt_reverseidx->second << "\t\t"
